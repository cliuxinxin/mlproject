...
class Doccano(object):
    def __init__(self):
        pc = ProjectConfig(os.path.join(ROOT_PATH,'config.ini'))
        url = pc.get_config('doccano','url')
        user = pc.get_config('doccano','user')
        password = pc.get_config('doccano','password')
        self.client = DoccanoClient(url,user,password)
