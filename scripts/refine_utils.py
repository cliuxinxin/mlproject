...
class Refine:
    ...
    def new_project(self, project_file=None, project_url=None, project_name=None, project_format='text/line-based/*sv',
        ...
        url_params = parse.parse_qs(
            parse.urlparse(response.geturl()).query)
...
class RefineProject:
    ...
    def do_json(self, command, data=None, include_engine=True):
        ...
        if 'historyEntry' in response:
            ...
            self.history_entry = HistoryEntry(he['id'], he['time'],
                                                      he['description'])
        return response
...
def create(project_file,
    ...
    project = Refine(RefineServer()).new_project(
